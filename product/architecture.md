...
Results are cached per project in memory. The cache is dropped and reloaded when the app becomes active, when the user hits Refresh in the project header, and — if the optional file watcher is built — when anything under the folder changes on disk. Saving `design.md` in an editor and seeing the Design tab update without touching the app is the workflow this exists for.

Load failures are not swallowed. A project whose documents failed to load says so in the project header and contributes a row to Monitor's broken lane.
