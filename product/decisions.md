...
Design is also the only tab that shows a document chip rail, because `ui-spec.md` routes to the Design tab too. A horizontal scroll view takes more height than the capsules inside it, which seated that rail below every other tab's first line, so it is sized to its content.
