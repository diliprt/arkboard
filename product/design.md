...
A heading buys air above itself, which is right in the middle of a document and wrong at the top of one: there the pane's padding is already that air. The first block a tab renders never carries it, including the block that became first when a repeated opener was skipped. Design is also the only tab with a document chip rail above its prose, because `ui-spec.md` routes to the Design tab too, and that rail is sized to its capsules rather than to a scroll view that would seat them lower than every other tab's first line.
