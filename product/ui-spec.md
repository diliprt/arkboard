...
Liquid Glass is the **navigation** layer and only the navigation layer: the sidebar, the window toolbar, the project tab rail, and the Contents inspector. The document is the **content** layer — solid, readable, and **never on glass**. A page of prose on glass is unreadable and is the one mistake this rule exists to prevent.

**Dark is a first-class look**, and it is where the split has to work hardest: a frosted rail and a solid page can land close enough in value that the window reads as one flat field. So every glass edge is drawn with a 1pt `separatorColor` hairline — the bottom of the tab rail, the top of the sidebar footer, the leading edge of Contents. A line, never a filled strip; a strip is opaque paint on navigation wearing a disguise. No wallpaper or photograph sits behind the sidebar: the material samples the desktop itself, which is the system's job.
